...
{foreach from = $SETTING  item = COLUMN}
    ...
    {elseif $COLUMN_NAME eq "listprice" }
        <td class="cellItem {if $COLUMN->isActive !='active'}hide{/if}"  >
            <div>
        ...
        <td class="cellItem customCell" >
            <div class="{$COLUMN_NAME}"  data-rowid="{$COLUMN_NAME}{$row_no}" data-lineitemtype = "{if !empty($data)}{$entityType}{else}{$BASE_ROW}{/if}">
                ...
                {else}
                    ...
                    {else}
                        <input type="text" value="{$data[$COLUMN_NAME|cat:$row_no]}" data-field-type="{$CUSTOM_COLUMN_SETTING[$COLUMN_NAME]->fieldType}" name="{$COLUMN_NAME|cat:$row_no}" id="{$COLUMN_NAME|cat:$row_no}" class="inputElement {$COLUMN_NAME}" style="width: 100%" />
                    {/if}
                {/if}
            </div>
        </td>
