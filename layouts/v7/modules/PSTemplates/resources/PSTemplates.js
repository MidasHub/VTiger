Vtiger.Class("PSTemplates_PSTemplates_Js",{
    ...
    getInstance: function() {
        ...
        this.self = new PSTemplates_PSTemplates_Js();
        return this.self;
    }
},{
    ...
    /*TECHNOKRAFTS*/
    tksGetModule :  function(){
        return jQuery('[name="module"]').val();
    },
    tksCalculateGrossProfit :  function(){
        //grandTotal = this.getGrandTotal()
        //var thisInstance = this;
        //var line_item = thisInstance.getLineItemContentsContainer();
        //var qty = line_item.find('.qty');
        //var total = 0;
        //var ourprice = 0;
        //jQuery.each(qty,function(index,domElement)
        //{
        //    if(index !=0)
        //    {
        //        var parentRow = jQuery(domElement).closest('tr.');
        //        var prod = parentRow.find('.productName');
        //        //alert(prod.val());
        //        if( typeof prod.val() == "undefined" || prod.val() == null || prod.val() == '' )
        //        {
        //            ourprice = 0;
        //            gross_profit =0;
        //            parentRow.find('.tks_grossprofit').text( '0.00' );
        //            parentRow.find('.tks_grossprofithidden').val( '0.00' );
        //        }
        //        else
        //        {
        //            var discountTotal = parentRow.find('.discountTotal');
        //
        //            var productTotal = parentRow.find('.productTotal').text();
        //            //var productTotal = grandTotal;
        //            var our_cost = parentRow.find('.tks_listpricehidden');
        //
        //            var q= domElement.value;
        //
        //            ourprice = ourprice + (parseFloat(our_cost.val()) * parseFloat(q));
        //
        //            gross_profit = parseFloat(productTotal) - parseFloat(discountTotal.text()) - ourprice;
        //
        //            gross_profit = gross_profit.toFixed(2);
        //            parentRow.find('.tks_grossprofit').text( gross_profit );
        //            parentRow.find('.tks_grossprofithidden').val( gross_profit );
        //            total = parseFloat(total) + parseFloat(gross_profit);
        //        }
        //        ourprice = 0;
        //        gross_profit =0;
        //
        ...
        //jQuery('#grandGrossProfitHidden').val(parseFloat(total).toFixed(2));
        //jQuery('#grandGrossProfit').text( parseFloat(total).toFixed(2) );
    },
    /*TECHNOKRAFTS*/
    ...
    setListPriceValue : function(lineItemRow, listPriceValue) {
        ...
        /*TECHNOKRAFTS*/
        if(this.tksGetModule() == 'Quotes' )
        {
            var tks_listlabel = jQuery('label.tks_listpricelabel',lineItemRow);
            var curr = jQuery('#currency_id option:selected');
            var currency_symbol = curr . text().substr(curr.text().indexOf('(') , curr.text().indexOf(')') );
            var tks_val = tks_listlabel.text().substr(0, tks_listlabel.text().indexOf(':')) + ': ' + currency_symbol.replace(/[()]/g,'') +' '+ listPrice;
            tks_listlabel.text( tks_val );
            jQuery('input.tks_listpricehidden',lineItemRow).val(listPrice);
        }
        /*TECHNOKRAFTS*/
        ...
        if(!jQuery.isEmptyObject(taxObj)){
            ...
            for(var taxName in taxObj){
                ...
                taxDiv +=
                    ...
                    '<input type="text" name="'+taxName+'_percentage'+rowNumber+'" data-rule-positive=true data-rule-inventory_percentage=true  id="'+taxName+'_percentage'+rowNumber+'" value="'+taxInfo.taxpercentage+'" class="taxPercentage span1" data-compound-on='+taxInfo.compoundOn+' >&nbsp;%'+
                    '</td>'+
    ...
    mapResultsToFields: function(referenceModule,element,responseData){
        ...
        for(var id in responseData){
            ...
            if(this.tksGetModule() == 'Quotes' )
            {
                var tks_listlabel = jQuery('label.tks_listpricelabel',parentRow);
                var curr = jQuery('#currency_id option:selected');
                var currency_symbol = curr . text().substr(curr.text().indexOf('(') , curr.text().indexOf(')') );
                var tks_val = tks_listlabel.text().substr(0, tks_listlabel.text().indexOf(':')) + ': ' + currency_symbol.replace(/[()]/g,'') +' '+ unitPrice;
                tks_listlabel.text( tks_val );
                jQuery('input.tks_listpricehidden',parentRow).val(unitPrice);
            }
            /*TECHNOKRAFTS*/
        ...
        if(referenceModule == 'Products'){
            ...
            inventoryInstance.loadSubProducts(parentRow);
        }
    ...
    updateLineItemsElementWithSequenceNumber : function(lineItemRow,expectedSequenceNumber , currentSequenceNumber){
        ...
        if(this.tksGetModule() == 'Quotes' )
        {
            ...
            var idFields = ['productName','subproduct_ids','hdnProductId',
                ...
                'productTotal','discountTotal','totalAfterDiscount','taxTotal','tks_listpricelabel',
                'tks_listpricehidden','tks_grossprofit','tks_grossprofithidden','tks_ourprice'];
            var nameFields = ['discount','tks_ourprice','tks_grossprofit'];
        }
        else
        {
    ...
    clearLineItemDetails : function(parentElem) {
        ...
        //{
        //    var tks_listlabel = jQuery('label.tks_listpricelabel',lineItemRow);
        //    var tks_val = tks_listlabel.text().substr(0, tks_listlabel.text().indexOf(':')) + ': ';
        //    tks_listlabel.text( tks_val );
        //
        //    thisInstance.tksCalculateGrossProfit();
        //    lineItemRow.find('.tks_grossprofit').text( '0.00' );
        //    lineItemRow.find('.tks_grossprofithidden').val( '0.00' );
        //}
        /*TECHNOKRAFTS*/
    },
    ...
    tksSetQty : function(qty,row) {
        var q =jQuery("#qty"+row);
        q.val(qty);
    },

    tksSetDiscounts : function(discount,row) {
        var tks_dis_type =jQuery("#discount_type"+row);
        tks_dis_type.val(discount[0].discount_type);
        var element =jQuery('#row'+row);
        var dis = element.find('.discounts');
        if( discount[0].discount_type == 'amount')
        {
            jQuery('[name="discount'+row+'"]')[2].checked = true;
            var dis_amount = jQuery("#discount_amount"+row);
            dis_amount.val(discount[0].discountTotal);
        }
        else
        {
            jQuery('[name="discount'+row+'"]')[1].checked = true;
            var dis_amount = jQuery("#discount_percentage"+row);
            dis_amount.val(discount[0].discount_percent);
        }

        var discountTotal =jQuery("#discountTotal"+row);
        discountTotal.val(discount[0].discountTotal);
    },

    tksAddingNewProductsRow: function(recordData,ind,discount,qty){
        var thisInstance = this;
        var lastrw = jQuery('#lineItemTab > tbody > tr:last');
        prod = lastrw.find('input.productName');
        ptype = lastrw.find('.lineItemPopup');
        seqno = thisInstance.getNextLineItemRowNumber();

        if(typeof prod.val() == "undefined" || prod.val() == null || prod.val() == '' && ptype.attr( "title" ) == 'Products' && ind == 0)
        {
            var popupElement = jQuery('.lineItemPopup', seqno-1 );
            seqno = seqno-1;
        }
        else
        {
        ...
        var popupElement = jQuery('.lineItemPopup',newRow);
        thisInstance.tksSetDiscounts(discount,seqno);
        thisInstance.tksSetQty(qty,seqno);
        thisInstance.mapResultsToFields('Products', popupElement, recordData);
    },

    tksAddingNewServicesRow: function(recordData,ind,discount,qty){
        var thisInstance = this;
        var lastrw = jQuery('#lineItemTab > tbody > tr:last');
        prod = lastrw.find('input.productName');
        ptype = lastrw.find('.lineItemPopup');
        ...
        if(typeof prod.val() == "undefined" || prod.val() == null || prod.val() == '' && ptype.attr( "title" ) == 'Services' && ind == 0)
        {
            var popupElement = jQuery('.lineItemPopup', seqno-1 );
            seqno = seqno-1;
        }
        ...
        thisInstance.tksSetDiscounts(discount,seqno);
        thisInstance.tksSetQty(qty,seqno);
        thisInstance.mapResultsToFields('Services', popupElement, recordData);
    },
    /**
     * Function to save the pre tax total value
     */
    registerTksSelectTemplateQuote : function() {
        var thisinstance = this;
        //var tks_select = jQuery('#tks_quotetemp');
        //tks_select.on('change',function()
        jQuery('body').on('change','#tks_quotetemp',function()
        {
            var tks_select = jQuery(this);
            recid = tks_select.find('option:selected').val();
            if(recid !=0 )
            {
                ...
                AppConnector.request(subProrductParams).then(
                    function(data)
                    {
                        ...
                        var responseData = JSON.parse(d.toString());
                        jQuery.each(responseData.result.item_list,function(index,ele) // haph86@gmail.com - #8090 - 05252015
                        {
                            if(ele['module'] == 'Products')
                            {
                                thisinstance.tksAddingNewProductsRow(ele.data,index,ele.discounts,ele.qty);
                            }
                            else if(ele['module'] == 'Services')
                            {
                                thisinstance.tksAddingNewServicesRow(ele.data,index,ele.discounts,ele.qty);
                            }
                        });
						
						// haph86@gmail.com - #8090 - 05252015
                        if(responseData.result.record_data.hdnTaxType) {
                            jQuery('#taxtype').val(responseData.result.record_data.hdnTaxType);
                            jQuery("#taxtype option[value="+responseData.result.record_data.hdnTaxType+"]").attr('selected', 'selected');
...
jQuery(document).ready(function(){
    ...
    if((module=='Quotes'||module=='Invoice'||module=='SalesOrder') && view=='Edit') {

        var actionParams = {
            "view":"PSTemplatesAjax",
            "mode":"listTemplates",
            "module":"PSTemplates"
        };
        app.request.get({data:actionParams}).then(
            function(err,data) {
                if(err == null && data) {
                    if (data.indexOf('activateLicenseForm') == -1){
                        var lineItemContent = jQuery('.lineitemTableContainer');
                        var lineItemHeader =lineItemContent.prev();

                        // jQuery('.fieldBlockHeader',lineItemHeader).first().parent().append(data);
                        // jQuery('.fieldBlockHeader',lineItemHeader).first().addClass('col-lg-7');
                        // jQuery('.fieldBlockHeader',lineItemHeader).first().next().addClass('col-lg-5');
                        // vtUtils.applyFieldElementsView(jQuery('.fieldBlockHeader',lineItemHeader).first().parent());
                        
                        var a = jQuery('.fieldBlockHeader',lineItemHeader).clone();
                        jQuery('.fieldBlockHeader',lineItemHeader).closest(".fieldBlockContainer").prepend(a);
                        var b = jQuery('.fieldBlockHeader',lineItemHeader).parent();
                        jQuery('.fieldBlockHeader', lineItemHeader).parent().css('margin-bottom', "10px");
                        jQuery('.fieldBlockHeader',lineItemHeader).parent().html(data);
                        vtUtils.applyFieldElementsView(b);
                    }
                }
            }
        );

        var PSTemplatesInstance = PSTemplates_PSTemplates_Js.getInstance();
        PSTemplatesInstance.registerTksSelectTemplateQuote();
    }


    //add icon for tools
    jQuery('.app-item-color-TOOLS').find('.app-icon-list.fa').addClass('fa-wrench');
});