...
<form name="editForm" class="form-horizontal" id="field-edit-form" ng-submit="saveThisRecord(editForm)" ng-controller="VtigerEditController" enctype="multipart/form-data" method="post">
    ...
    <section layout="row" flex class="content-section">
        <div layout="column" class="edit-content" layout-fill layout-align="top center">
            <div ng-repeat="block in blocksData" style="border-radius: 4px;border: 1px solid #ddd; margin: 7px 7px 0px;">
                <div style="padding: 16px;color: #333;background-color: #f5f5f5;border-bottom: 1px solid #ddd;border-top-left-radius: 4px;border-top-right-radius: 4px;">
                    <span>{{block}}</span>
                </div>

                <md-list class="fields-list">
                    <md-list-item ng-repeat="field in fieldsData" class="md-1-line" ng-if="field.editable && block == field.block">
                        <div class="md-list-item-text field-row" ng-switch="field.type.name">
                            ...
                            <md-input-container ng-switch-when="picklist" ng-hide="(field.name == 'activitytype' || field.name == 'eventstatus') && module =='Calendar'">
                                <div class="input-group-addon">
                                    ...
                                    <md-select name="{{field.name}}" ng-focus ="focusCallback($event)"  ng-change="registerEventForPicklistDependencySetup()"  ng-model="field.raw" aria-label="{{field.label}}" ng-required="field.mandatory">
                            ...
                            <div ng-switch-when="reference" style="padding-bottom: 16px;">
                                <div class="input-group-addon">
                                    <label>{{field.label}}</label>
                                    <md-autocomplete ng-if="field.name !='folderid'" name="{{field.name}}" flex
                                                     ng-model="field.raw"
                                    ...
                                    <md-select name="{{field.name}}" ng-if="field.name == 'folderid'" ng-model="field.raw" aria-label="{{field.label}}" ng-required="field.mandatory">
                                        <md-option ng-value="opt.value" ng-repeat="opt in field.type.picklistValues">{{opt.label}}</md-option>
                                    </md-select>
                                </div>
                            ...
                            <!--*************Upload field**********************-->
                            <md-input-container ng-switch-when="file">
                                <div class="input-group-addon">
                                    <label>{{field.label}}</label>
                                    <input type='file' ng-model="field.raw" onchange="angular.element(this).scope().fileNameChanged(this)" name='{{field.name}}' id='file' aria-label="{{field.name}}">
