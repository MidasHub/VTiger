            ...
            <optgroup label='{vtranslate($PRIMARY_MODULE_NAME,$MODULE)}-{vtranslate($BLOCK_LABEL,$PRIMARY_MODULE_NAME)}'>
                {foreach key=FIELD_KEY item=FIELD_LABEL from=$BLOCK}
                    ...
                    {if $FIELD_INFO[4] eq 'D' or $FIELD_INFO[4] eq 'DT'}
                        ...
                        <option value="{$FIELD_KEY}:W">{vtranslate($FIELD_LABEL, $PRIMARY_MODULE_NAME)} ({vtranslate('LBL_WEEK', $PRIMARY_MODULE_NAME)})</option>
                        {if $FIELD_INFO[1] eq 'modifiedtime' || $FIELD_INFO[1] eq 'createdtime'}
                        <option value="{$FIELD_KEY}:D">{vtranslate($FIELD_LABEL, $PRIMARY_MODULE_NAME)} ({vtranslate('LBL_DAY', $PRIMARY_MODULE_NAME)})</option>
                        {/if}
                        <option value="{$FIELD_KEY}">{vtranslate($FIELD_LABEL, $PRIMARY_MODULE_NAME)}</option>
                    {elseif $FIELD_INFO[4] neq 'I' and $FIELD_INFO[4] neq 'N' and $FIELD_INFO[4] neq 'NN'}
                        <option value="{$FIELD_KEY}">{vtranslate($FIELD_LABEL, $PRIMARY_MODULE_NAME)}</option>
                    {/if}
                {/foreach}
            </optgroup>
            ...
            <optgroup label='{vtranslate($SECONDARY_MODULE_NAME,$MODULE)}-{vtranslate($BLOCK_LABEL,$SECONDARY_MODULE_NAME)}'>
                {foreach key=FIELD_KEY item=FIELD_LABEL from=$BLOCK}
                    ...
                    {if $FIELD_INFO[4] eq 'D' or $FIELD_INFO[4] eq 'DT'}
                        ...
                        <option value="{$FIELD_KEY}:W">{vtranslate($SECONDARY_MODULE_NAME, $SECONDARY_MODULE_NAME)} {vtranslate($FIELD_LABEL, $SECONDARY_MODULE_NAME)} ({vtranslate('LBL_WEEK', $SECONDARY_MODULE_NAME)})</option>
                        {if $FIELD_INFO[1] eq 'modifiedtime' || $FIELD_INFO[1] eq 'createdtime'}
                        <option value="{$FIELD_KEY}:D">{vtranslate($SECONDARY_MODULE_NAME, $SECONDARY_MODULE_NAME)} {vtranslate($FIELD_LABEL, $SECONDARY_MODULE_NAME)} ({vtranslate('LBL_DAY', $SECONDARY_MODULE_NAME)})</option>
                        {/if}
