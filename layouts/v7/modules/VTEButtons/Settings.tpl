...
<div class="container-fluid WidgetsManage">
    ...
    <div class = "row">
        ...
        <div class="col-md-3">
            ...
            <div class="col-md-9">
                <select class="inputElement select2"  id='filterText' style='display:inline-block' onchange='filterText(this)'  >
                    <option value="all">{vtranslate('All', 'VTEButtons')} ({($MODULE_NUMB)})</option>
                    ...
                    {foreach item=MODULE_VALUES from=$ALL_MODULES}
                       {* {if in_array($MODULE_VALUES->name, $EXCLUDE_MODULE_ARRAY)}
                        ...
                        <option value="{$MODULE_VALUES->name}" {if $MODULE_VALUES->name eq $RECORDENTRIES['module']}selected{/if}>{vtranslate($MODULE_VALUES->label,$MODULE_VALUES->name)} ({($record_numb[$MODULE_NAME])})</option>
                    {/foreach}
                </select> 
            </div>
        </div>
                ...
                <table id="myTable" class="table listview-table">
                    ...
                    {foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES}
                        ...
                        <tr class="listViewEntries content {$LISTVIEW_ENTRY['module']}" data-url = {{$MODULE_MODEL->getCreateViewUrl($LISTVIEW_ENTRY['id'])}}>
                            <td style="    width: 75px; padding-left: 15px; vertical-align: top">
