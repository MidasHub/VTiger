...
//                    calcLeft = ((itemDiff / minuteDiff) * 100) - (itemDiff/minuteDiff);
    ...
    SetupItemEvents: function (itemElem) {
        ...
        if (TimeScheduler.Options.AllowDragging && (itemData.module == 'Events' || itemData.module == 'Calendar')) {
            itemElem.draggable({
                helper: 'clone',
                zIndex: 1,
        ...
        if (TimeScheduler.Options.AllowResizing && itemData.module == 'Events' || itemData.module == 'Calendar') {
    ...
    roundTimeWithDuration: function (timeObj) {
        ...
        if(time_minutes % duration != 0) {
            ...
            timeObj = timeObj.add('minutes', time_minutes).subtract('minute', current_time_minutes);
            return timeObj;
        }else{
            return timeObj;
        }
    ...
    GotoTimeShift_Clicked: function (event) {
        ...
        var dt = $('<input type="text" id="go_to_date_' + currentTimeStamp + '" />')
            ...
            .datepicker({
                'autoclose': true,
                onClose: function () {alert('xxxxx');
                    $(this).remove();
                },
                onSelect: function (date) {
                    TimeScheduler.Options.Start = moment(date).add('minutes', period.StartPeroid * 60);
                    TimeScheduler.Init();
                },
                defaultDate: TimeScheduler.Options.Start.format('YYYY-MM-DD')
            }).on('changeDate', function (e) {
                TimeScheduler.Options.Start = moment(e.date).add('minutes', period.StartPeroid * 60);
                TimeScheduler.Init();
            });
    ...
    TimeShift_Clicked: function (event) {
        ...
        if ($(this).is('.time-sch-time-button-today')) {
            ...
            if(period.Name == '1 week'){
                TimeScheduler.Options.Start = current_start.startOf('week');
                TimeScheduler.Options.Start.subtract('weeks', 1);
            }else if(period.Name == '1 month'){
                TimeScheduler.Options.Start = current_start.startOf('month');
                TimeScheduler.Options.Start.subtract('months', 1);
            }else if(period.Name == '1 days'){
                TimeScheduler.Options.Start.subtract('days', 1);
                TimeScheduler.Options.Start.startOf('day').add('minutes', period.StartPeroid * 60);
            }
            ...
            if(period.Name == '1 week'){
                TimeScheduler.Options.Start = current_start.startOf('week');
                TimeScheduler.Options.Start.add('weeks', 1);
            }else if(period.Name == '1 month'){
                TimeScheduler.Options.Start = current_start.startOf('month');
                TimeScheduler.Options.Start.add('months', 1);
            }else if(period.Name == '1 days'){
                TimeScheduler.Options.Start.add('days', 1);
                TimeScheduler.Options.Start.startOf('day').add('minutes', period.StartPeroid * 60);
            }
    ...
    SelectPeriod: function (name) {
        ...
        if(name == '1 week'){
            ...
            var date_end_moment = moment(date_end, dateFormat.toUpperCase()).add('minutes', 24*60);
    ...
    registerCustomDateFilter: function (periodContainer) {
        ...
        periodContainer.on('datepicker-change', 'a[data-calendar-type="range"]', function(e){
            ...
            if(diff_days > 31){
                app.helper.showErrorNotification({title: app.vtranslate('Limited 31 days'), message: app.vtranslate('Date range should be in a month to show activities better')});
                return;
            }else {
                jQuery('a', periodContainer).removeClass('time-sch-selected-button');
