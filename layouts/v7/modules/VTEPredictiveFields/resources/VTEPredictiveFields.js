...
Vtiger.Class("VTEPredictiveFields_Js", {
    ...
    registerApplyPredictiveOnQuickCreate:function(module){
        var self = this;
        var params = {};
        var list_fields = [];
        var field_module = module;
        params['module'] = 'VTEPredictiveFields';
        params['action'] = 'ActionAjax';
        params['mode'] = 'getPredictions';
        params['view'] = 'QuickCreate';
        ...
        app.request.post({data:params}).then(function(err,data) {
            if(err == null){
                ...
                $.each(list_fields,function (index,field_name) {
                    var btn_predictive = self.getPredictionForField(data,field_name,'','QuickCreate');
                    var this_field = $("#QuickCreate [name='"+field_name+"']");
    ...
    getPredictionForField:function (data,field_name,record_id,quickCreate) {
        var predictions = [];
        ...
        if(quickCreate != undefined && quickCreate != ''){
            view = 'quickCreate';
        }
        var self = this;
        ...
        if(view == 'quickCreate'){
            var this_field = $("#QuickCreate input[data-name='"+field_name+"']");
            if(this_field.length == 0) this_field = $("#QuickCreate [name='"+field_name+"']");
        }
        var current_value = "";
        ...
        jQuery.each( data[field_name], function(key,cval ) {
            if(predictions.length == 0){
                ...
                if(field_type  == "date") {
                    ...
                    if(current_value == "" || current_value == " " || current_value == undefined || typeof current_value == "undefined"){
                        var postvalue1_int = cval[6];
                        var postvalue2_int = cval[7];
                        var postvalue3_int = cval[8];
                        var postvalue4_int = cval[9];
            ...
            $(predictions).each(function (key, val) {
                if(val){
                    ...
                    if(val[0] && val[1] && sugest_value && sugest_value[0] !=val[0]){
                        if(view == 'quickCreate'){
                            var itemClass = 'linkToUpdatePridictionQuickCreate';
                        }else{
                            var itemClass = 'linkToUpdatePridiction';
                        }
                        if(field_type == 'date'){
                            if(int_value != 0){
                                predictions_html += "<a class='"+itemClass+"' data-fieldname='" +field_name+ "'  data-module='" +field_module+ "' data-recordid='" +record_id+ "' data-value='" +(field_type != 'picklist' ? val[0] : int_value)+ "'>" +val[0]+(field_type == 'date' ? " ("+(int_value >=0 ? '+'+int_value : int_value)+' '+ (int_value == 1 || int_value == -1 ? 'day' : 'days')+")" : '')+"</a><br/>";
                            }
                        }else{
                            predictions_html += "<a class='"+itemClass+"' data-fieldname='" +field_name+ "'  data-module='" +field_module+ "' data-recordid='" +record_id+ "' data-value='" +(field_type != 'picklist' ? val[0] : int_value)+ "'>" +val[0]+(field_type == 'date' ? " ("+(int_value >=0 ? '+'+int_value : int_value)+' '+ (int_value == 1 || int_value == -1 ? 'day' : 'days')+")" : '')+"</a><br/>";
                        }
    ...
    PredictiveUpdateThisClickQuickCreate:function(e){
        var this_btn = $(e);
        var link_text = this_btn.text();
        var module = this_btn.data('module');
        ...
        var this_field = $("#QuickCreate [name='"+apply_field+"']");
        var fieldtype = this_field.data('fieldtype');
        if(typeof fieldtype == "undefined"){
            if(this_field.hasClass('sourceField')){
                this_field = $("#QuickCreate [data-fieldname='"+apply_field+"']");
        ...
        $("#QuickCreate button[data-fieldname='" + apply_field +"']").hide();
    },
                                ...
                                if(typeof parent_span.data('field-type') != "undefined" && parent_span.data('field-type') == "reference"){
                                    ...
                                    if(field_td_on_detail_view.find('span.value').find('span').length > 0){
                                        if(parent_span.data('field-type') == 'date'){
                                            ...
                                            if(data.picklist_color != undefined && data.picklist_color != ''){
                                                field_td_on_detail_view.find('span.value').find('span').addClass('picklist-color');
                                                field_td_on_detail_view.find('span.value').find('span').css({'background-color': data.picklist_color,'line-height': '15px','color': data.text_color});
                                            }
                                        }else{
                                            field_td_on_detail_view.find('span.value').find('span').html(newValue);
                                        }
                                        ...
                                        if(parent_span.data('field-type') == 'date'){
                                            ...
                                            if(data.picklist_color != undefined && data.picklist_color != ''){
                                                field_td_on_detail_view.find('span.value').addClass('picklist-color');
                                                field_td_on_detail_view.find('span.value').css({'background-color': data.picklist_color,'line-height': '15px','color': data.text_color});
                                            }
                                        }else{
                                            field_td_on_detail_view.find('span.value').html(newValue);
                                        }
    ...
    registerPredictiveUpdateThisClick: function () {
        ...
        $(document).on('click','.predictiveUpdateThis',function() {
            ...
            if(view == 'quickCreate'){
                self.PredictiveUpdateThisClickQuickCreate(this);
            }else{
                self.PredictiveUpdateThisClick(this);
            }
        });
        $(document).on('click','.linkToUpdatePridictionQuickCreate',function() {
            self.PredictiveUpdateThisClickQuickCreate(this);
...
jQuery(document).ready(function () {
    ...
    jQuery( document ).ajaxComplete(function(event, xhr, settings) {
        ...
        if(other_url.view == 'QuickCreateAjax'){
            instance.registerApplyPredictiveOnQuickCreate(other_url.module);
        }
