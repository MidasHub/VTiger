...
-->*}
{strip}
	<script type="text/javascript" src="layouts/v7/modules/RealTimeFieldFormulas/resources/RealTimeFieldFormulas.js"></script>

	<div class="row">
		<div class="col-sm-2 col-xs-2">
			<label>
				<input type="checkbox" name="ActiveRealTimeField" {if $TASK_OBJECT->checkExecuteDemand($TASK_ID) eq true || empty($TASK_ID)}checked="checked"{/if}> Execute on demand.
			</label>
		</div>
		<div class="col-sm-10 col-xs-10 row" style="background-color: rgba(222, 190, 0, 0.18); padding: 0px">
			<p style="padding: 5px 15px; margin: 0px">If "Execute on Demand" is enabled, the updates/formulas will execute right away (on the screen).</p>
		</div>
	</div>
