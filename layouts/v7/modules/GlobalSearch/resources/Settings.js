...
Vtiger.Class("GlobalSearch_Settings_Js",{
    ...
    registerSaveSettings:function() {
        ...
        jQuery("#global_search_settings").on("click",".btnSaveSettings", function(e) {
            ...
            var selectedQuickSearchFields = self.getSelectedColumns('quick_search_fields_show');
            $('#selectedQuickSearchFields').val(selectedQuickSearchFields);
            var selectedSearchFields = self.getSelectedColumns('quick_search_fields_search');
            $('#selectedSearchFields').val(selectedSearchFields);
            var selectedModuleFields = self.getSelectedColumns('moduleFields');
            $('#selectedModuleFields').val(selectedModuleFields);
        ...
        jQuery("#QuickSearch").on("click","#save_search_method", function(e) {
            app.helper.showProgress();
            var search_method = $('#search_method').val();
            var limit_per_module = $('#limit_per_module').val();
            var params = {};
            params['module'] = 'GlobalSearch';
            params['action'] = 'ActionAjax';
            params['mode'] = 'updateSearch_method';
            params['search_method'] = search_method;
            params['limit_per_module'] = limit_per_module;
            app.request.post({'data' : params}).then(
                function(err,data) {
                    if(data) {
                        app.helper.hideProgress();
    ...
    makeS2SortAble:function(select2Element){
    ...
    makeColumnListSortable : function() {
        ...
        $(document).ajaxComplete(function(){
            var select2Element_1 = jQuery('#s2id_moduleFields');
            var select2Element_2 = jQuery('#s2id_moduleFieldsShow');
            var select2Element_3 = jQuery('#s2id_quick_search_fields_search');
            var select2Element_4 = jQuery('#s2id_quick_search_fields_show');
            self.makeS2SortAble(select2Element_1);
            self.makeS2SortAble(select2Element_2);
            self.makeS2SortAble(select2Element_3);
            self.makeS2SortAble(select2Element_4);
        });
    },
