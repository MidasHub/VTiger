...
function initData_GlobalSearch() {
    ...
    $(document).ajaxComplete(function(event, jqxhr, settings){
        if(settings.data != undefined && typeof settings.data == 'string' && settings.data.indexOf('view=BasicAjax') != -1 && settings.data.indexOf('mode=showAdvancedSearch') != -1){
                ...
                if(search_module != undefined && search_module != ''){
                    $(document).ajaxComplete(function(event, jqxhr, settings){
                        if(settings.data != undefined && settings.data.indexOf('view=ListAjax') != -1 && settings.data.indexOf('mode=showSearchResults') != -1){
                            ...
                            window.location.href = "index.php?module=GlobalSearch&view=SearchResults&search_module="+search_module+'&value=&search_type=vt_standard&search_params='+search_params;
                        }
                    });
                }
            });
...
function ajaxShowLogged(){
    ...
    app.request.post({'data' : params}).then(function(err, data){
        if(err == null){
            ...
            $('.search-ajax-record').hover(function(){
                $(this).find('.open-new-tab').removeClass('hidden');
            },function(){
                $(this).find('.open-new-tab').addClass('hidden');
            });
        }
    });
}
