...
{strip}
    ...
    <div id="modnavigator" class="row module-nav">
        <div style="width: 42px;float: left;margin-left: 15px;" class="hidden-xs hidden-sm mod-switcher-container">
            {include file="modules/Vtiger/partials/Menubar.tpl"}
        </div>
    </div>
    <div style="" class="main-container clearfix">
        <input type='hidden' value="{$VALUE}" id='searchKey'>
        <input type='hidden' value="{$KEY_WORD_SEARCH}" id='keyWordSearch'>
        <input type='hidden' value='{$SEARCH_PARAMS}' id='search_params'>
        <input type='hidden' value="{$SEARCH_TYPE}" id='search_type'>
        <div class="clearfix main-content">
            <div style="float: left;margin-left: 42px" class="left-content col-lg-2">
                <div class="left-content-module-list">
                    ...
                    {if $FOUND_RECORDS eq 0}
                        <div><h5 style="text-align: center;font-size: 12px;">No records found</h5></div>
                    {else}
                        <br>
                    {/if}
                    <span class="label">Keyword search</span>
                    <div class="left-content-search-link hidden-xs">
                        <span class="fa fa-search" aria-hidden="true"></span>
                        <input style="border: 0;height: 30px;width: 70%;margin: 1px 0 1px 5px;" class="global-keyword-input" type="text" placeholder="Type keyword..." value="">
                    </div>
            ...
            <div class="right-content pull-left" style="margin-left: 21%">
                <div class="scroll">
                    ...
                    {else}
                        ...
                        {foreach item=LIST_RESULTS key=SEARCH_MODULE from=$SEARCH_RESULTS name=listresult}
                            ...
                            <div class="module-search-result listViewTopMenuDiv noprint" id="{$SEARCH_MODULE}_search_content">
                                ...
                                </span><span class="totalNumberOfSearchResults cursorPointer" data-module="{$SEARCH_MODULE}" title="Click for this list size">{vtranslate('LBL_OF',$SEARCH_MODULE)} <i class="fa fa-question showTotalCountIcon"></i></span>
                                            &nbsp;&nbsp;&nbsp;<a id="listViewPreviousPageButton{$SEARCH_MODULE}" class="previousPageButton listViewPreviousPageButton navigationButton " data-start='{$PAGING_MODEL->getRecordStartRange()-$PAGING_MODEL->getPageLimit()}'data-module="{$SEARCH_MODULE}" {if !$PAGING_MODEL->isPrevPageExists()}disabled=""{/if}>
                                                <i class="fa fa-caret-left"></i>&nbsp;&nbsp;
                                            </a>
                                            <a id="listViewNextPageButton{$SEARCH_MODULE}" class="nextPageButton listViewNextPageButton navigationButton " data-module="{$SEARCH_MODULE}" data-start='{$PAGING_MODEL->getRecordEndRange()}' {if !$PAGING_MODEL->isNextPageExists()} disabled=""{/if}>
