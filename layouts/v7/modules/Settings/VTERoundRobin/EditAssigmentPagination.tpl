...
<div class = "{$CLASS_VIEW_ACTION}">
    <div class="btn-group pull-right">
        <a href="index.php?module=VTERoundRobin&parent=Settings&view=Edit&record={$RECORD_DATA['record']}&page={($PAGE_NUMBER -1)}" type="button" id="PreviousPageButton" class="btn btn-default" {if !$PAGING_MODEL->isPrevPageExists()} disabled {/if}><i class="fa fa-caret-left"></i></a>
        {if $SHOWPAGEJUMP}
        ...
        {/if}
        <a href="index.php?module=VTERoundRobin&parent=Settings&view=Edit&record={$RECORD_DATA['record']}&page={($PAGE_NUMBER +1)}" type="button" id="NextPageButton" class="btn btn-default" {if !$PAGING_MODEL->isNextPageExists()}disabled{/if}><i class="fa fa-caret-right"></i></a>
    </div>
    <span class="pageNumbers  pull-right" style="position:relative;top:7px;">
        ...
        &nbsp;<span class="totalNumberOfRecords cursorPointer{if !$RECORD_COUNT} hide{/if}" title="{vtranslate('LBL_SHOW_TOTAL_NUMBER_OF_RECORDS', $MODULE)}">{vtranslate('LBL_OF', $MODULE)} <a href="javascript:void(0)" class="showTotalCountIcon" data-value="{$ROUNDROBIN_ASSIGMENTS_COUNT}"><i class="fa fa-question"></i></a></span>&nbsp;&nbsp;
    </span>
</div>