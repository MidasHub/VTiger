...
{strip}
    ...
    <div class="editViewPageDiv">
        ...
        <form id="EditView" action="index.php" method="post" name="EditSLAPolicies">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <select class="inputElement select2" id="business_hour" name="business_hour">
                                <option value="Yes" {if $RECORD_DETAILS['slaBusinessHour'] eq 'Yes'}selected="selected"{/if} >{vtranslate('LBL_YES',{$QUALIFIED_MODULE})}</option>
                                <option value="No" {if $RECORD_DETAILS['slaBusinessHour'] neq 'Yes'}selected="selected"{/if}>{vtranslate('LBL_NO',{$QUALIFIED_MODULE})}</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
            ...
            {*SLA Trigger/Condition*}
            <div class="col-sm-12 col-xs-12">
                <div id="hide_picklist_field" class="hide">
                    {if $RECORD_DETAILS['record']}
                        <option value="">Select an Option</option>
                        {foreach item=FIELD_LABEL key=FIELD_NAME from=$FIELDS_PICKLIST_MODULE}
                            <option value="{$FIELD_NAME}"
                                    {if $RECORD_DETAILS['slaPickListField'] == $FIELD_NAME}selected="selected"{/if}>{$FIELD_LABEL}</option>
                        {/foreach}
                    {/if}
                </div>
                <input type="hidden" id="input_hide_picklist_field">
                <h4 class="label-block">{vtranslate('SLA Trigger/Condition',{$QUALIFIED_MODULE})}</h4>
                <hr/>
                <div class="col-sm-6 col-xs-6 form-horizontal">
                    <div class="form-group">
                        <label for="picklist_field" class="control-label col-sm-4" style="padding-right:0px">
                            <span>{vtranslate('LBL_PICKLIST_FIELD',{$QUALIFIED_MODULE})}</span>
                            <span class="redColor"> *</span>
                            <i class="cursorPointer fa fa-info-circle pull-right" data-toggle="tooltip" title="" data-original-title="Select the value, which will be used to trigger the SLA."></i>
                        </label>
                        <div class="col-sm-7">
                            <select class="inputElement select2" id="picklist_field"
                                ...
                                <option VALUE="">{vtranslate('LBL_SELECT_OPTION',{$QUALIFIED_MODULE})}</option>
                                {if $RECORD_DETAILS['record']}
                                    <option value="None" {if $RECORD_DETAILS['slaPickListField'] == 'None'}selected="selected"{/if}>None</option>
                                    {foreach item=FIELD_LABEL key=FIELD_NAME from=$FIELDS_PICKLIST_MODULE}
                                        <option value="{$FIELD_NAME}"
                                                {if $RECORD_DETAILS['slaPickListField'] == $FIELD_NAME}selected="selected"{/if}>{$FIELD_LABEL}</option>
                                    {/foreach}
                    ...
                    <div class="form-group">
                        <label for="sla_picklist_field_value"
                               class="control-label col-sm-4" style="padding-right: 0;">
                            <span>{vtranslate('LBL_RESOLVED_WITHIN',{$QUALIFIED_MODULE})}</span>
                            <span class="redColor"> *</span>
                            <i class="cursorPointer fa fa-info-circle pull-right"  data-toggle="tooltip" title='Resolved within should be set to the number of hours/minutes needed/allocated to resolve the issue. For exmaple, if your SLA is 5 hours for "Critical" ticket, then you should set Resolved Within to 5 hours.'></i>
                        </label>
                        <div class="col-sm-7">
                ...
                <div class="col-sm-6 col-xs-6">
                    <div class="form-group">
                        <span class="message_sla_trigger">
                            {if $RECORD_DETAILS['slaPickListField'] == 'None' && $RECORD_DETAILS['slaPickListValue'] == 'None'}
                                SLA will trigger <b>for all records created (starting from now)</b> and is expected to <strong class="text_int_time">{if $RECORD_DETAILS['slaResolvedTime']}{$RECORD_DETAILS['slaResolvedTime']}{else}??{/if}</strong> <strong class="text_typetime">{if $RECORD_DETAILS['slaResolvedTypeTime']}{$RECORD_DETAILS['slaResolvedTypeTime']}{else}??{/if}</strong>
                            {else}
                                SLA will trigger when <strong class="trigger_fieldlabel">{if $RECORD_DETAILS['slaPickListField']}{Settings_VTESLAPolicies_Module_Model::getLabelField($RECORD_DETAILS['slaPickListField'],$MODULE_MODEL->getName())}{else}??{/if}</strong> is <strong class="triggervalue">{if $RECORD_DETAILS['slaPickListValue']}{$RECORD_DETAILS['slaPickListValue']}{else}??{/if}</strong> and is expected to <strong class="text_int_time">{if $RECORD_DETAILS['slaResolvedTime']}{$RECORD_DETAILS['slaResolvedTime']}{else}??{/if}</strong> <strong class="text_typetime">{if $RECORD_DETAILS['slaResolvedTypeTime']}{$RECORD_DETAILS['slaResolvedTypeTime']}{else}??{/if}</strong>
                            {/if}
                        </span>
                    </div>
                </div>
            ...
            <div class="col-sm-12 col-xs-12" style="margin-bottom: 20px;">
                <button type="button" class="btn btn-default" name="modify_condition" data-val="1">
                    <strong>Custom Conditions</strong>&nbsp;&nbsp;
                    <i class="fa fa-chevron-right"></i>
                </button>

                <div class="count_active_condition" style="color: red; font-weight: bold;font-style: italic;display: inline;margin-left: 50px;"><span class="active_condition" style="color: red;font-weight: bold;font-style: italic"></span> Active condition(s)</div>
            </div>
            <div class="col-sm-10 col-xs-10 hide" id="advanceFilterContainer">
            ...
            {*SLA Status Control*}
            <div class="col-sm-12 col-xs-12">
                <div id="hide_exclude_field" class="hide">
                    {if $RECORD_DETAILS['record']}
                        {foreach item=VALUE_FIELD from=$VALUE_PICKLIST_FIELD_EXCLUDE}
                            <option value="{$VALUE_FIELD}">{vtranslate($VALUE_FIELD,$RECORD_DETAILS['slaModule'])}</option>
                        {/foreach}
                    {/if}
                </div>
                <input type="hidden" id="input_exclude_field_value">
                <h4 class="label-block">{vtranslate('SLA Status Control',{$QUALIFIED_MODULE})}</h4>
                <hr/>
                <div class="col-sm-6 col-xs-6 form-horizontal">
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <select class="inputElement select2" id="exclude_field"
                                ...
                                {if $RECORD_DETAILS['record']}
                                    {foreach item=FIELD_LABEL key=FIELD_NAME from=$FIELDS_PICKLIST_MODULE}
                                        <option value="{$FIELD_NAME}"
                                                {if $RECORD_DETAILS['slaExcludeField'] == $FIELD_NAME}selected="selected"{/if}>{$FIELD_LABEL}</option>
                                    {/foreach}
                                {/if}
                            </select>
                        </div>
                ...
                <div class="col-sm-6 col-xs-6">
                    {assign var="valueStartContinue" value=array_diff($VALUE_PICKLIST_FIELD_EXCLUDE,$RECORD_DETAILS['slaExcludeFieldValue'])}
                    {$valueStartContinue = array_diff($valueStartContinue,$RECORD_DETAILS['fulfillment_values'])}
                    <div class="form-group">
                        <span>SLA Timer will start and/or continue to run when <strong class="statusfield_label">{if $RECORD_DETAILS['slaExcludeField']}{Settings_VTESLAPolicies_Module_Model::getLabelField($RECORD_DETAILS['slaExcludeField'],$MODULE_MODEL->getName())}{else}??{/if}</strong> is <strong class="value_not_selected_in_pause_or_fullfilment">{if count($valueStartContinue) > 0}{implode(',',$valueStartContinue)}{else}??.{/if}</strong></span>
                    </div>
                    <div class="form-group">
                        <span>SLA Timer will pause when <strong class="statusfield_label">{if $RECORD_DETAILS['slaExcludeField']}{Settings_VTESLAPolicies_Module_Model::getLabelField($RECORD_DETAILS['slaExcludeField'],$MODULE_MODEL->getName())}{else}??{/if}</strong> is <strong class="holdvalues">{if $RECORD_DETAILS['slaExcludeFieldValue']}{implode(',',$RECORD_DETAILS['slaExcludeFieldValue'])}{else}??{/if}.</strong></span>
                    </div>
                    <div class="form-group">
                        <span>SLA Timer will stop/complete when <strong class="statusfield_label">{if $RECORD_DETAILS['slaExcludeField']}{Settings_VTESLAPolicies_Module_Model::getLabelField($RECORD_DETAILS['slaExcludeField'],$MODULE_MODEL->getName())}{else}??{/if}</strong> is <strong class="fullfilment_value">{if $RECORD_DETAILS['fulfillment_values']}{implode(',',$RECORD_DETAILS['fulfillment_values'])}{else}??{/if}.</strong></span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-sm-12 col-xs-12">
                ...
                <table class="table" id="sla_actions">
                    ...
                    <tr>
                        <th></th>
                        <th>{vtranslate('LBL_NAME_SLA_ACTION',{$QUALIFIED_MODULE})}&ensp;<i class="cursorPointer fa fa-info-circle"  data-toggle="tooltip" title='Alert Name - can be anything.' ></i></th>
                        <th>{vtranslate('LBL_TYPE',{$QUALIFIED_MODULE})}&ensp;<i class="cursorPointer fa fa-info-circle"  data-html="true" data-toggle="tooltip" title="Select from 3 options: <br><br> Email - email will be sent to the selected user.<br><br>Reassign - record will be reassigned to selected user. </br></br>Workflow - selected workflow will get executed on the active record. Please see documentation for more details." ></i></th>
                        <th>{vtranslate('LBL_TRIGGER_SLA_ACTION',{$QUALIFIED_MODULE})}&ensp;<i class="cursorPointer fa fa-info-circle"  data-html="true" data-toggle="tooltip" title='Select from 2 options:</br></br>Before - meaning that you can trigger action BEFORE the SLA has been breached. For example, if your SLA policy is "resolve within 8 hours", you can trigger email before you hit the 8 hour mark. You can send an email to manager/supervisor 2 hours BEFORE, so they can make sure the problem is being worked on and will get resolved in the next 2 hours (so you do not violate the SLA).</br></br>After - meaning that you can trigger action AFTER SLA has been breached. '></i></th>
                        <th>{vtranslate('LBL_USER',{$QUALIFIED_MODULE})} &ensp;<i class="cursorPointer fa fa-info-circle"  data-toggle="tooltip" title='Depending on TYPE, this selection will be used to send an email (to selected user) OR assign active record (to selected user).' ></i></th>
                        <th>{vtranslate('LBL_EMAIL_WORKFLOW',{$QUALIFIED_MODULE})}&ensp;<i class="cursorPointer fa fa-info-circle"  data-toggle="tooltip" title='Select existing email template OR workflow. Please see documentation for more details.' ></i></th>
                        <th></th>
                    </tr>
                    </thead>
                    <tbody>
