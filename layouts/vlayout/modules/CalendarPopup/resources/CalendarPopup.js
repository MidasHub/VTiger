 ...
 * All Rights Reserved.
 * ****************************************************************************** */
jQuery.Class("CalendarPopup_Edit_Js",{
    editInstance:false,
            ...
            AppConnector.request(data).then(
                function(data) {
                    if(data.indexOf('SUCCESS') != -1) {
                        ...
                        var actionParams = {
                            "type":"POST",
                            "url":'index.php?module=CalendarPopup&view=MassActionAjax&mode=showCalendarPopup&record='+eventId,
                            "dataType":"html",
                            "data" : {}
                        };
                                        ...
                                        for (i = 0; i < listModules.length; i++) {
                                            if(listModules[i] == 'Calendar'){
                                                ...
                                                if(selectedRecord == '') {
                                                    ...
                                                    app.destroyChosenElement(massEditForm);
                                                }
                                            }
                                        }
        ...
        if(mode == 'create' && (source_module == 'Contacts' || source_module == 'Potentials')) {
            ...
            if(accountForm.length>0) {
                ...
                if(source_module == 'Contacts') {
                    ...
                    data['related_to']=accountid;
                }
            }
        }
        var url ='index.php?module=CalendarPopup&view=MassActionAjax&mode=getEditForm&source_module='+source_module+'&record='+record+'&editmode='+mode;
        var actionParams = {
            "type":"POST",
            "url":url,
            "dataType":"html",
...
//                jQuery("#module_"+source_module+"_Fields").html(data);
                ...
                if(mode == '') {
                    ...
                    app.registerEventForDatePickerFields(massEditForm);
                }
                massEditForm.validationEngine(app.validationEngineOptions);
                var editInstance = Vtiger_Edit_Js.getInstanceByModuleName(source_module);
                editInstance.registerBasicEvents(massEditForm);
                if(mode == 'create') {
                    if(massEditForm.find('input[name="account_id"]').val() !='' && massEditForm.find('input[name="account_id"]').val() !='0') {
                        var accountData={};
                        ...
                        editInstance.referenceSelectionEventHandler(accountData, massEditForm);
                    }
                }
            }
        );
...
//        fieldElement.trigger(Vtiger_Edit_Js.referenceSelectionEvent, {'source_module' : popupReferenceModule, 'record' : id, 'selectedName' : selectedName});
    ...
    getReferenceSearchParams : function(element){
        var tdElement = jQuery(element).closest('div.referenceField');
        var params = {};
    ...
    registerEventForCreateAnotherBtn : function (container) {
        ...
        container.on('click', '.btnCreateAnother', function (e) {
            ...
            AppConnector.request(actionParams).then(
                function(data) {
                    container.find('div.massEditContent').append(data);
...
//console.log(jQuery("div.activityEntries a[class!='fieldValue']"));
    jQuery(document).on("click", "a.fc-event, tr.listViewCalendar, div.activityEntries a[class!='fieldValue']", function(e) {
        ...
        if(jQuery(this).is('a')) {
            ...
            var orgUrl=jQuery(this).data('recordurl');
        }
                    ...
                    if(data == 'Events') {
                        ...
                        AppConnector.request(actionParams).then(
                            function(data) {
                                ...
                                if(data) {
                                    app.showModalWindow(data,function(data){
                                        app.showScrollBar(jQuery('div[name="massEditContent"]'), {'height':'600px'});
