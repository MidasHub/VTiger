    ...
    <div class="row-fluid referenceField">
        ...
        <div style="margin-top: 5px;" class="span4">&nbsp;
            {if $MODULE_LABEL eq 'Leads'}
                <button onclick="javascript:CalendarPopup_Edit_Js.convertLead('index.php?module=CalendarPopup&view=ConvertLead&eventid={$EVENTID}',this);" id="Leads_detailView_basicAction_LBL_CONVERT_LEAD" class="btn"><strong>{vtranslate('LBL_CONVERT_LEAD', $MODULE_LABEL)}</strong></button>
            {/if}
        </div>
        <div style="margin-top: 5px;" class="span4">
        ...
        <div style="margin-top: 5px;" class="span4">&nbsp;
            {if $MODULE_LABEL eq 'Contacts' || $MODULE_LABEL eq 'Potentials'}
                <button class="btn btn-success btnCreateAnother pull-right" data-module="{$MODULE_LABEL}" data-module-label="{vtranslate('SINGLE_'|cat:$MODULE_LABEL, $MODULE_LABEL)}"><strong>{vtranslate('LBL_CREATE_ANOTHER', 'CalendarPopup')} {vtranslate('SINGLE_'|cat:$MODULE_LABEL, $MODULE_LABEL)}</strong></button>
            {/if}
        </div>
    </div>
