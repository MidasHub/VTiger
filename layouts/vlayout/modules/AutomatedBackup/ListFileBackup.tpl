            ...
            <td style="text-align: center;" class="listViewEntryValue medium" nowrap="">
                {if $LISTVIEW_ENTRY.deleted eq 0}
                    {if $LOG_TYPE eq 'ftpbackup'}
                        <a class="downloadRecordButton" title="Download" data-file="{$FTP_DOWNLOAD_LINK}/{$LISTVIEW_ENTRY.path}/{$LISTVIEW_ENTRY.filename}"><i title="Download" class="icon-download alignMiddle"></i></a>
                    {else}
                        <a class="downloadRecordButton" title="Download" data-file="{$SITE_URL}/{$LISTVIEW_ENTRY.path}/automatedbackup_{$LISTVIEW_ENTRY.filename}"><i title="Download" class="icon-download alignMiddle"></i></a>
                    {/if}
                {/if}
            </td>
        </tr>
