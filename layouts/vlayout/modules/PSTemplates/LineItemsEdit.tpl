...
{strip}
    ...
    <table class="table table-bordered blockContainer lineItemTable" id="lineItemTab">
        ...
        <tr id="row0" class="hide lineItemCloneCopy">
            {include file="LineItemsContent.tpl"|@vtemplate_path:'PSTemplates' row_no=0 data=[]}
        </tr>
        {foreach key=row_no item=data from=$RELATED_PRODUCTS}
            <tr id="row{$row_no}" class="lineItemRow" {if $data["entityType$row_no"] eq 'Products'}data-quantity-in-stock={$data["qtyInStock$row_no"]}{/if}>
                {include file="LineItemsContent.tpl"|@vtemplate_path:'PSTemplates' row_no=$row_no data=$data}
            </tr>
        {/foreach}
        {if count($RELATED_PRODUCTS) eq 0}
            <tr id="row1" class="lineItemRow">
                {include file="LineItemsContent.tpl"|@vtemplate_path:'PSTemplates' row_no=1 data=[]}
            </tr>
        {/if}
    ...
    <table class="table table-bordered blockContainer lineItemTable" id="lineItemResult" style="display:none">
        <tr>
            <td  width="83%">
		...
		<tr style="display:none"><!--TECHNOKRAFTS-->
			<td width="83%">
		...
		<!-- Group Tax - starts -->
        <tr id="group_tax_row" valign="top" class="{if $IS_INDIVIDUAL_TAX_TYPE}hide{/if}"><!--TECHNOKRAFTS-->
            <td width="83%">
