...
<thead>
	<tr class="listViewHeaders">
		<th width="5%" class="{$WIDTHTYPE}">
			<strong>{vtranslate('Product and Service Templates',$MODULE)}</strong>
		</th>
	</tr>
<thead>
</table>		
...
<div class="listViewEntriesDiv contents-bottomscroll">
	...
	<table class="table table-bordered listViewEntriesTable" >
		...
		<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getEditViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
			...
			<td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" data-field-name="{$LISTVIEW_HEADER->getFieldName()}" nowrap>
				{if $LISTVIEW_HEADER->isNameField() eq true or $LISTVIEW_HEADER->get('uitype') eq '4'}
					<a href="{$LISTVIEW_ENTRY->getEditViewUrl()}">{$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}</a>
