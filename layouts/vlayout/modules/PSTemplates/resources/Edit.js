 ...
 * All Rights Reserved.
 *************************************************************************************/
Vtiger_Edit_Js("PSTemplates_Edit_Js",{
	...
	isIndividualTaxMode : function() {
		...
		if(selectedOption.val() == PSTemplates_Edit_Js.individualTaxType){
			return true;
		}
		return false;
	},
	...
	isGroupTaxMode : function() {
		var taxTypeElement = this.getTaxTypeSelectElement();
		var selectedOption = taxTypeElement.find('option:selected');
		if(selectedOption.val() == PSTemplates_Edit_Js.groupTaxType){
			return true;
		}
		return false;
	},
	...
	calculateDiscountForLineItem : function(lineItemRow) {
		...
		if(discountType == PSTemplates_Edit_Js.percentageDiscountType){
		...
		}else if(discountType == PSTemplates_Edit_Js.directAmountDiscountType){
...
//va
    ...
    loadItemDetail: function (targetModule) {
        if(targetModule != '' && targetModule != undefined){
            lineItemTab = jQuery('#lineItemTab');
			var recordId = lineItemTab.closest('.editViewContents').find('#recordId').val()
            params={
                module:'Quoter',
                view:'MassActionAjax',
                mode: 'getItemsEdit',
            ...
            AppConnector.request(params).then(
                function(data){
                    ...
                    if(response.result.isActive == true){
						...
						jQuery(".verticalBottomSpacing").show();
